<template>
    <div class="regist-comp">
        ...
        <van-tabs class="content tours-tabs-nowrap" @change="changeTabs">
            <!-- 手机号注册 -->
            <van-tab class="regist" :title="$t('partcailComp.phoneReg')">
                <van-cell-group>
                    <!-- 手机号 -->
                    <area-code-input class="phone" :proAreaCode.sync="phoneForm.areaCode" :proMobile.sync="phoneForm.phone" />
                    <!-- 密码 -->
                    <van-field class="password tours-input" v-model="phoneForm.password" center clearable icon="eye-o" :placeholder="$t('partcailComp.placePassword')" :type="pswInputType" @click-icon="toggleInputType">
                    </van-field>
                    <!-- 验证码 -->
                    <van-field class="password tours-input" v-model="phoneForm.smsCode" center clearable placeholder="验证码">
                        <van-button class="tours-button-noborder" slot="button" size="small" @click="getCode(LOGIN_WAY.PHONE)">{{showText}}</van-button>
                    </van-field>
                </van-cell-group>
            </van-tab>
            <!-- 请输入邮箱 -->
            <van-tab class="email-regist" :title="$t('partcailComp.emailReg')">
                <van-cell-group>
                    <van-field class="email tours-input" v-model="emailForm.email" :placeholder="$t('email')" />
                    <van-field class="password tours-input" v-model="emailForm.password" center clearable icon="eye-o" :placeholder="$t('partcailComp.placePassword')" :type="pswInputType" @click-icon="toggleInputType">
                    </van-field>
                    <van-field class="auth-code tours-input" v-model="emailForm.emailCode" center clearable :placeholder="$t('verifyCode')">
                        <van-button class="tours-button-noborder" slot="button" size="small" :disabled="codeType===VERIFY_CODE.GETTING" @click="getCode(LOGIN_WAY.WMAIL)">{{showText}}</van-button>
                    </van-field>
                </van-cell-group>
            </van-tab>
            <!-- 按钮 -->
            <div class="to-login" v-if="showLoginTip">
                <span>{{$t('partcailComp.hoveUser')}}</span>
                <span class="blue" @click="toLogin">{{$t('partcailComp.nowLogin')}}</span>
            </div>
            <van-button class="btn-regist tours-button" size="large" :disabled="!checked" :loading="submiting" @click="btnRegist">{{$t('regist')}}</van-button>
            <div class="text">
