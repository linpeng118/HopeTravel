  ...
  <div class="product-detail-page" ref="refProductDetailPage">
    ...
    <div class="product-detail" ref="refProductDetail" v-if="product">
      ...
      <div class="product">
        <p class="setelse">
          <span>
            <span v-if="product.comment_score !== '0.0'"
              >{{ product.comment_score }}分</span
            >
            <span v-else>暂无评论</span>
          </span>
          &nbsp;&nbsp;
          <span v-if="product.sales > 0">出行人数：{{ product.sales }}</span>
        </p>
        <!-- name -->
        <div class="name">
          <div class="title">
            <div class="prod-tag" v-if="product.self_support">
              {{ $t("selfSupport") }}
            </div>
        ...
        <div class="price-wrap">
          ...
          >
            <img
              src="../../assets/imgs/union/icon_share@2x.png"
              alt=""
              width="16"
              height="16"
            />
            <span
              >{{ $t("productDetailPage.shareMakes")
              }}{{ product.agent_fee }}</span
            >
          </span>
          <span
            class="price fs-48 fw-800"
          ...
          <div
            ...
            @click="downloadApp"
          >
            <img src="../../assets/imgs/phone2.png" alt="" />App下单立减{{
              product.newer_min_discount
            }}起,更有新人优惠等你拿
          </div>
        </div>
      </div>
      ...
      <div class="destination mt-24">
        ...
        >
          <div class="item-list">
            <span class="item-titlex"
              >{{ $t("productDetailPage.getCoupon") }}&nbsp;&nbsp;&nbsp;</span
            >
            <span
        ...
        <div
          ...
          v-show="
            product.product_entity_type ||
              product.tour_category === 'Unassigned'
          "
        >
          <div class="from">
      ...
      <!-- 团期价格 -->
      <div
        class="group-price mt-24"
        v-show="
          product.product_entity_type || product.tour_category === 'Unassigned'
        "
      >
        <div class="title">
        ...
        <div class="content mt-12">
          ...
          >
            <p class="time">
              {{ item.month }}/{{ item.day }}&nbsp;{{
                getWeek(item.year, item.month, item.day)
              }}
            </p>
      ...
      <div class=" mt-24 comment__wrapper" v-if="reviews">
        <h1>
          <b v-if="reviews.comment.score !== '0.0'"
            >{{ $t("comment.rate") }} {{ reviews.comment.score }}</b
          >
          <b v-else>暂无评论</b>
          <span @click="jumpTo(`/comment/detail/${product.product_id}`)"
            >{{ $t("seeAll") }}（{{ reviews.product.comment_count }}）
            <van-icon name="arrow"
          /></span>
        </h1>
        <p>
        ...
        <p
          class="comment__cnt"
          @click="jumpTo(`/comment/detail/${product.product_id}`)"
        >
      ...
      >
        ...
        <div v-if="hasFeature">
          <img
            v-for="item in product.feature_images"
            :key="item"
            :src="item"
            alt="img"
            width="100%"
      ...
      <div class="trip" ref="refTrip">
        ...
        >
          <h3 class="header-title">
            {{ $t("productDetailPage.travelProfile") }}
          </h3>
          <div class="header-content">
            ...
            <div class="item">
              ...
              <p>
                ...
                }}{{ $t("productDetailPage.scenicSpot") }}
              </p>
            </div>
            <div class="item">
        ...
        <div class="content-wrap">
          <div class="content-title">
            {{ $t("productDetailPage.itineraryDetail") }}
          </div>
          <div
            class="content"
          ...
          >
            ...
            <div class="detail">
              ...
              <div class="summarize-wrap">
                <h3 class="title">
                  <img
                    src="../../assets/imgs/product/text@2x.png"
                    alt="icon"
                  />&nbsp;{{ $t("productDetailPage.general") }}
                </h3>
                <div class="body" v-html="item.content"></div>
              </div>
              <!-- 地点 -->
              <div
                class="attractions-wrap"
              ...
              <div
                class="meals-wrap"
                v-if="
                  item.meal.breakfast.contain_meal == 1 ||
                    item.meal.lunch.contain_meal == 1 ||
                    item.meal.dinner.contain_meal == 1
                "
              >
                <h3 class="title">
                  <img
      ...
      >
        ...
        <span class="custom" @click="onAdCustom">{{
          $t("productDetailPage.customText")
        }}</span>
      </div>
      <!-- 费用明细 -->
      <div
      ...
      >
        ...
        <div class="price-desc">
          <h3 class="title-s">
            {{ $t("productDetailPage.priceDescription") }}
          </h3>
        ...
        <div class="price-inexclude">
          ...
          <div
            class="newData"
            v-if="
              expense.package_include_list.length > 0 ||
                expense.package_exclude_list.length > 0
            "
          >
            ...
            <ul class="price-newInclude">
              <li
                v-for="(item, index) in expense.package_include_list"
                :key="index"
              >
            ...
            <ul class="price-newExclude">
              <li
                v-for="(item, index) in expense.package_exclude_list"
                :key="index"
              >
                <span class="title">{{ item.title }}</span>
                <span class="content">{{ item.content }}</span>
          ...
          <div class="oldData" v-else>
            ...
            <van-collapse v-model="priceExclude">
              <van-collapse-item
                :title="$t('productDetailPage.feeExcludes')"
                name="exclude"
              >
      ...
      <div class="footer-fixed">
        <div class="footer-tabbar">
          <div class="operate">
            <div
              class="btn-operate"
              v-for="(item, index) in operateTabbar"
              :key="item.name"
              :data-agl-cvt="index == 1 ? 2 : index == 2 ? 1 : ''"
          ...
          <div class="reserve">
            <van-button
              class="btn-reserve"
              :style="{
                background: product.self_support ? '#EF9A1A' : '#fb605d'
              }"
              size="large"
              @click="btnReserve"
              >{{
      ...
      <!-- 服务说明 -->
      <van-action-sheet
        v-model="showServiceNode"
        :title="$t('productDetailPage.serviceDescription')"
        class="service-note"
      >
        <div
          class="servive-item mt-50"
          v-for="(item, index) in serviceNote"
          :key="index"
        >
          <h3 class="title">
            <img
              src="../../assets/imgs/product/tick@2x.png"
              alt="icon"
            />&nbsp;{{ item.title }}
          </h3>
        ...
        >
          ...
          <div class="cupright">
            <span
              ...
              v-if="item.is_receivable === true"
              >{{ $t("productDetailPage.receive") }}</span
            >
            <span
              class="btn2"
              v-if="item.is_received === true && item.is_receivable === false"
              >{{ $t("productDetailPage.haveReceive") }}</span
            >
          </div>
        </div>
      </van-action-sheet>
    ...
    <div
      v-if="
        isShareBtn && profile.is_agent && product.agent_fee.substring(1) > 0
      "
...
export default {
  ...
  computed: {
    ...
    tabList() {
      return [
        ...
        },
        {
          id: 2,
          name:
            this.itinerary.duration_days + this.$t("productDetailPage.dayTrip"),
          ref: "refTrip",
          isShow: true
        },
        {
      ...
      return [
        {
          ...
          icon: this.product.is_favorite
            ? require("../../assets/imgs/product/star_active@2x.png")
            : require("../../assets/imgs/product/star@2x.png")
        },
        {
    ...
    showDayList() {
      ...
      let newData = (this.itinerary.items || []).map(
        item => this.$refs[`refContent${item.sort_order}`][0].offsetTop
      );
      return newData;
    },
    topPrice() {
  ...
  beforeDestroy() {
    this.$refs.refProductDetailPage.removeEventListener(
      "scroll",
      this.scrollFn
    );
  },
  ...
  methods: {
    ...
    async initProfileData() {
      setTimeout(() => {
        ...
        if (
          this.profile.is_agent &&
          String(this.$route.query.productId).indexOf("-") <= 0
        ) {
          this.$router.replace({
            name: "product-detail",
            query: {
              productId:
                String(this.$route.query.productId) +
                "-" +
    ...
    scrollFn() {
      ...
      let tabListH =
        this.$refs.refTabList.offsetTop - this.$refs.refTabList.offsetHeight;
      let tabHeightH =
        this.$refs.refTabHeight.offsetTop - this.$refs.refTabList.offsetHeight;
      let homeHeight = this.$refs.refProductDetailPage.getBoundingClientRect()
        .height;
    ...
    async shareProductHandle() {
      if (!this.profile.customer_id) {
        ...
        let code = await getCode(
          `${window.location.origin}/product/detail?productId=${product_id}-${customer_id}`
        );
        this.shareDataInfo = {
          product_id,
          name,
    ...
    backTop() {
      let timer = setInterval(() => {
        ...
        this.$refs.refProductDetailPage.scrollTop =
          this.$refs.refProductDetailPage.scrollTop + speed;
