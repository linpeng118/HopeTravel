<template>
    <div class="product-detail-page" ref="refProductDetailPage">
        <!-- 头部 -->
        <product-detail-header class="product-detail-header" v-if="product" :transparent="isTransparent" :title="product.name" fixed @callOnRight="onHeaderRight" @callOnLeft="onHeaderLeft" ref="refProdctDetailHeader" />
        <div class="product-detail" ref="refProductDetail" v-if="product">
            <!-- banner -->
            <div class="banner-wrap">
                <van-swipe class="banner" :autoplay="4000" indicator-color="white" @change="onBannerChange">
...
:style="{'background': `url(${image}) no-repeat 0 0/100% 100%`}"></div> -->
            ...
            <div class="destination mt-24">
                <div class="item-wrap" @click="onServerCop" v-if="couponList&&couponList.length">
                    <div class="item-list">
                ...
                <div class="item-wrap" @click="onServerNode" v-if="serviceNote.length">
                    <div class="item-list">
                        <div class="item" v-for="(item,index) in serviceNote" :key="index">
                            <img src="../../assets/imgs/product/tick@2x.png" alt="icon">
                            {{item.title}}
                        </div>
                    </div>
                    <div class="item-arrow">
                ...
                <!-- 出发地结束地 -->
                <div class="city" v-show="product.product_entity_type || product.tour_category==='Unassigned'">
                    <div class="from">
                        <img src="../../assets/imgs/product/from@2x.png" alt="from">
                        <span class="title">{{$t('productDetailPage.origin')}}</span>
                        <span class="addr">{{product.departure_city}}</span>
                    </div>
                    <div class="to">
                        <img src="../../assets/imgs/product/to@2x.png" alt="from">
            ...
            <!-- 团期价格 -->
            <div class="group-price mt-24" v-show="product.product_entity_type || product.tour_category==='Unassigned'">
                <div class="title">
                    <img src="../../assets/imgs/product/price@2x.png" alt="icon">
                    {{$t('productDetailPage.groupPrice')}}
                </div>
                <div class="content mt-12">
                    <div v-for="(item,index) in topPrice" :key="index" class="g-price-item" @click="onGroupPriceDate(item)">
                        <p class="time">
            ...
            <div class="recommend mt-24">
                <div class="title">
                    <img src="../../assets/imgs/product/praise@2x.png" alt="icon">
                    {{$t('tours.torusRecommend')}}
                </div>
                <div class="content mt-12" v-html="product.small_description"></div>
            </div>

            <!--评论版块-->
            <div class=" mt-24 comment__wrapper" v-if="reviews">
                <h1>
                ...
                <p class="comment__cnt" @click="jumpTo(`/comment/detail/${product.product_id}`)">{{reviews.comment.content}}</p>
            </div>

            <!-- tab触发则滚动 -->
            <div class="tab-list-wrap" :class="{'fixed-tab': isTabFixed}" v-show="product.product_entity_type || product.tour_category==='Unassigned'">
                <div class="tab-list" ref="refTabList">
                    <div class="tab-item" v-for="tab in tabList" :key="tab.name" :class="{'active': activeTab===tab.id}" @click="clickTab(tab)" v-show="tab.isShow">
                        <span>{{tab.name}}</span>
                        <span class="tab-day" v-show="tab.id===2">{{showDay}}</span>
                    </div>
                </div>
            </div>
            <div class="tab-height mt-24" ref="refTabHeight" v-show="isTabFixed && (product.product_entity_type || product.tour_category==='Unassigned')"></div>
            <!-- 产品特色 -->
            <div class="features" ref="refFeatures" v-show="product.product_entity_type || product.tour_category==='Unassigned'">
                <!-- :style="{'background': `url(${bgFeat}) no-repeat 0 0/100% 100%`}"> -->
                <div v-if="hasFeature">
                    <img v-for="item in product.feature_images" :key="item" :src="item" alt="img" width="100%">
                </div>
            </div>
            <!-- 行程概要 -->
            <div class="trip" ref="refTrip">
                <div class="header-wrap" v-show="product.product_entity_type || product.tour_category==='Unassigned'">
                    ...
                    <div class="header-content">
                        <div class="item">
                            <img src="../../assets/imgs/product/trip_1@2x.png" alt="trip">
                            <p>{{itinerary.duration_days}}{{$t('productDetailPage.dayTrip')}}</p>
                        </div>
                        <div class="item">
                            <img src="../../assets/imgs/product/trip_2@2x.png" alt="trip">
                            <p>{{itinerary.total_attractions}}{{$t('productDetailPage.scenicSpot')}}</p>
                        </div>
                        <div class="item">
                            <img src="../../assets/imgs/product/trip_3@2x.png" alt="trip">
                ...
                <div class="content-wrap">
                    ...
                    <div class="content" v-for="item in itinerary.items" :key="item.product_itinerary_id" :ref="`refContent${item.sort_order}`">
                        ...
                        <div class="detail">
                            ...
                            <div class="summarize-wrap">
                                <h3 class="title">
                                    <img src="../../assets/imgs/product/text@2x.png" alt="icon">&nbsp;{{$t('productDetailPage.general')}}
                                </h3>
                                <div class="body" v-html="item.content"></div>
                            </div>
                            <!-- 地点 -->
                            <div class="attractions-wrap" v-for="attr in item.attractions" :key="attr.tour_city_id">
                                <h3 class="title">
                                    <img src="../../assets/imgs/product/text@2x.png" alt="icon">&nbsp;{{attr.name}}
                                </h3>
                                <div class="body">
                                    <div class="attr-imgs">
                                        <prod-detail-img-item :proData="attr.images" @callOnSlide="onImgSlide" />
                                    </div>
                                    <div class="desc" v-html="attr.content"></div>
                                </div>
                            </div>
                            <!-- 酒店 -->
                            <div class="hotel-wrap" v-if="item.hotel">
                                <h3 class="title">
                                    <img src="../../assets/imgs/product/text@2x.png" alt="icon">&nbsp;{{$t('productDetailPage.hotel')}}
                                </h3>
                                <div class="body" v-html="item.hotel"></div>
                            </div>
                            <!-- 餐食（contain_meal：1包含，2不包含） -->
                            <div class="meals-wrap" v-if="item.meal.breakfast.contain_meal==1 || item.meal.lunch.contain_meal==1 || item.meal.dinner.contain_meal==1">
                                <h3 class="title">
                                    <img src="../../assets/imgs/product/text@2x.png" alt="icon">&nbsp;{{$t('productDetailPage.meals')}}
                                </h3>
                                <div class="body">
                                    <p class="breakfast" v-if="item.meal.breakfast.contain_meal==1">
                                    ...
                                    <p class="lunch" v-if="item.meal.lunch.contain_meal==1">
                                        {{$t('productDetailPage.lunch')}}：
                                        <span v-if="item.meal.lunch.remark">{{item.meal.lunch.remark}}</span>
                                        <span v-else>{{$t('productDetailPage.include')}}</span>
                                    </p>
                                    <p class="dinner" v-if="item.meal.dinner.contain_meal==1">
            ...
            <div class="ad-custom" v-show="product.product_entity_type || product.tour_category==='Unassigned'">
                ...
                <span class="custom" @click="onAdCustom">{{$t('productDetailPage.customText')}}</span>
            </div>
            <!-- 费用明细 -->
            <div class="cost" ref="refCost" v-show="product.product_entity_type || product.tour_category==='Unassigned'">
                <h1 class="title">
                    {{$t('costDetail')}}
                </h1>
                <div class="group-price-desc">
                    ...
                    <div class="price-item-wrap">
                        <div class="price-item" v-for="item in standartPrice" :key="item.name">
            ...
            <div class="notice mt-24" ref="refNotice">
                <van-collapse v-model="activeNotice" accordion>
                    <van-collapse-item v-for="(item, index) in notice" :key="item.title" :title="item.title" :name="index+1">
            ...
            <div class="code-two">
                ...
                <div class="btn-box" v-clipboard:copy="'zmcslxs'" v-clipboard:success="onCopy" v-clipboard:error="onError">
                    复制公众号
                </div>
            </div>
            <!--悬浮-->
            <drift-aside ref="driftAside" :showContactCall="false" @backTop="backTop"></drift-aside>
            <!-- 底部按钮 -->
            <div class="footer-fixed">
                <div class="footer-tabbar">
                    <div class="operate">
                        <div class="btn-operate" v-for="(item,index) in operateTabbar" :key="item.name" :data-agl-cvt="index==1?2:index==2?1:''" @click="onOperate(item)">
                            <img :src="item.icon" alt="icon">
                            <p class="operate-name">{{item.name}}</p>
                        </div>
                    </div>
                    <div class="reserve">
                        <van-button class="btn-reserve" :style="{'background': product.self_support ? '#EF9A1A' : '#fb605d'}" size="large" @click="btnReserve">{{product.is_soldout
                ...
                <!-- 咨询引导弹层 -->
                <div class="call-guide" @click.stop="callguideClose()" v-if="closeSelf!=1">
                    没有找到合适的产品，让饭饭帮我订
                    <van-icon name="cross" />
                </div>
            ...
            <<van-action-sheet v-model="showServiceNode" :title="$t('productDetailPage.serviceDescription')" class="service-note">
                ...
                <<van-action-sheet v-model="showServiceCop" :title="$t('coupons')" class="service-note">
                    ...
                    <div class="cup-item" v-for="(item,index) in couponDetails" :key="index">
                        ...
                        <div class="cupright">
                            <span class="btn1" @click="getcouponobj(item.id)" v-if="item.is_receivable === true">{{$t('productDetailPage.receive')}}</span>
                            <span class="btn2" v-if="item.is_received === true && item.is_receivable === false">{{$t('productDetailPage.haveReceive')}}</span>
                        </div>
                    </div>
                    </van-action-sheet>
                    <!-- 恢复预定通知 -->
                        ...
                        <div class="sold-out-content">
                            ...
                            <div class="account-wrap mt-30">
                                <van-cell-group class="account-input">
                                    <van-field class="tours-input" v-model="account" :placeholder="$t('plhdNameEmail')">
                                    </van-field>
                                </van-cell-group>
                                <van-button class="account-btn" slot="button" size="small" @click="btnOrder">{{$t('submit')}}</van-button>
                            </div>
                        </div>
                        </van-action-sheet>
        </div>
        ...
        <van-popup v-model="isVideoShow" position="right">
            <van-nav-bar title="" left-arrow @click-left="pausePlay" />
    ...
    export default {
        ...
        computed: {
            ...
            standartPrice() {
                ...
                newData.forEach(item => {
                    if (
                        this.expense.standard_price &&
                        this.expense.standard_price[item.type] &&
                        this.expense.standard_price[item.type]
                    ) {
            ...
            operateTabbar() {
                return [{
                        ...
                        icon: this.product.is_favorite ?
                            require('../../assets/imgs/product/star_active@2x.png') : require('../../assets/imgs/product/star@2x.png'),
                    },
                    {
        ...
        methods: {
            ...
            async ashbackLogic() {
                ...
                if (query.indexOf('-') >= 0) {
                    ...
                    if (navigator.userAgent.indexOf('MicroMessenger') >= 0) {
                        ...
                        navigator.userAgent.indexOf('QBWebViewType') >= 0 ||
                        navigator.userAgent.indexOf('MQQBrowser') >= 0
                    ) {
            ...
            backFn() {
                // 滚动到的元素(减去2个fixed的高度)
                let scrollTo =
                    this.$refs[this.activeTabRef].offsetTop -
