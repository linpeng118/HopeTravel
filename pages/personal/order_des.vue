<template>
    <section>
        <van-nav-bar class="login-header tours-no-bb" ref="loginHeader" :title="$t('orderDetailPage.title')" :z-index="999" @click-left="onClickLeft">
        ...
        <!--评价按钮-->
        <div class="comment__create" v-if="details.status.code==2 && details.product_comment_status==0" @click.stop="linkToCreate(details)">去评价，最高可得500米粒哟~</div>
        <div class="comment__create" v-if="details.status.code==2 && details.product_comment_status==900003" @click.stop="linkToCreate(details)">追加评价</div>
    </section>
</template>
