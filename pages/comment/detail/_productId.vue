        ...
        <div class="commentDetail__wrapper">
            ...
            <div v-if="RateCountsData">
                <div class="commentDetail__rateList" v-if="RateCountsData.score_info.length>1">
                    <div class="rate__total">
                        ...
                        <span>综合评分</span>
                        <van-rate v-model="scoreFormat" allow-half void-icon="star" color="#EF9A1A" void-color="#eee" :size="16" readonly />
                    </div>
                    <ul class="rateList__bars">
                        <li v-for="(item,index) in RateCountsData.score_info" :key="index">
                            <b>{{item.name}}</b>
                            <van-progress class="rateList__bars__progress" :percentage="(parseFloat(item.score)/5)*100" color="#EF9A1A" :show-pivot="false" />
                ...
                <div class="commentDetail__rateList__single" v-else>
                    <span>综合评分</span>
                    <van-rate v-model="scoreFormat" allow-half void-icon="star" color="#EF9A1A" void-color="#eee" :size="24" gutter="20px" readonly />
                    <b>{{RateCountsData.comment_score}}</b>
                </div>
            </div>
            ...
            <div v-if="RateCountsData && RateCountsData.comment_count">
                ...
                <ul class="commentDetail__cate" v-if="RateCountsData">
                    <li @click="filterListHandle(0)" class="commentDetail__cate__item" :class="{on:cateIndex===0}">全部评价({{RateCountsData.comment_count}})</li>
                    <li @click="filterListHandle(1)" class="commentDetail__cate__item" :class="{on:cateIndex===1}" v-if="RateCountsData.top">精华({{RateCountsData.top}})</li>
                    <li @click="filterListHandle(2)" class="commentDetail__cate__item" :class="{on:cateIndex===2}" v-if="RateCountsData.good">好评({{RateCountsData.good}})</li>
                    <li @click="filterListHandle(3)" class="commentDetail__cate__item" :class="{on:cateIndex===3}" v-if="RateCountsData.bad">差评({{RateCountsData.bad}})</li>
                    <li @click="filterListHandle(4)" class="commentDetail__cate__item" :class="{on:cateIndex===4}" v-if="RateCountsData.image">有图({{RateCountsData.image}})</li>
                    <li @click="filterListHandle(5)" class="commentDetail__cate__item" :class="{on:cateIndex===5}" v-if="RateCountsData.append">追评({{RateCountsData.append}})</li>
                </ul>
                <!-- 评论列表 -->
                <van-list class="commentList" v-model="loading" :finished="finished" finished-text="没有更多了" @load="fetchList">
                    <ul>
                        <li v-for="item in LIST" :key="item.comment_id + Math.random()">
                            ...
                            <div class="rate">{{item.score}} <span v-if="item.is_top == 1">精华</span></div>
                            <!--评论-->
                            <div class="content">{{item.content}}</div>
                            <!--图片列表-->
                            <div class="img-list" v-if="item.image && item.image.length">
                                <img v-for="(item2,index2) in item.image" :src="item2" alt="" :key="index2" @click="imagePreviewHandle(item.image,index2)" />
                            </div>
                            ...
                            <div v-if="item.append_comment">
                                ...
                                <!--图片列表-->
                                <div class="img-list" v-if="item.append_comment.image && item.append_comment.image.length">
                                    <img v-for="(item3,index3) in item.append_comment.image" @click="imagePreviewHandle(item.append_comment.image,index3)" :src="item3" alt="" :key="index3" />
                                </div>
                            </div>
                            <!--点赞-->
                            <div class="prise" :class="{on:item.is_praise!=0}">
                                <div class="praise-box" @click="praiseHanlde(item)">
                                    <van-icon name="thumb-circle-o" /><span>有用 {{item.praise}}</span>
                                </div>
                            </div>

                            <!--稀饭回复-->
                            <div class="reply" v-if="item.reply">
                                <div class="append-tit tourscool"><img src="~assets/imgs/tourscool.png" alt="">稀饭回复</div>
...
<script>
    ...
    export default {
        ...
        methods: {
             ...
             */
            async fetchList() {
                let res = await getCommentList(this.$route.params.productId, this.listParams)
